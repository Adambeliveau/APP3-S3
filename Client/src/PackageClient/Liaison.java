...
public class Liaison {
    ...
    private Boolean sendPackets(byte[] packetCRC, int cpt) throws IOException {
        ...
        byte[] buf = new byte[1];
        DatagramPacket confirmation = new DatagramPacket(buf, 1, address, 32367);
        socket.receive(confirmation);
        String confirmationS = new String(confirmation.getData());
        String cptS = Integer.toString(cpt);
        if (cptS.equals(confirmationS)) {
            confi = true;
        } else {
            confi = false;
        }
